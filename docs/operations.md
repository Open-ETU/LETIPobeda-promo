...
Если у аккаунта нет права purge, workflow предупреждает об этом: новый HTML
появится по истечении 60-секундного кеша. Остальные пути CDN очищать не нужно: имена ассетов меняются при изменении контента.
